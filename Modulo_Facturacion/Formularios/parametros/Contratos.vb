...
Public Class Contratos
    ...
    Dim BanderaComboBoxPrincipales As Int32 = 0
...
#Region "Botones y texbox"
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click 'Anula el contrato
        Try
            Dim estado As Boolean = ClaseModeloContratos.EliminarContrato(txtIdContrato.Text)
            If estado Then
    ...
    Private Sub btnNuevo_Click(sender As Object, e As EventArgs) Handles btnNuevo.Click
        Try
            ...
            DataGridContratos.ClearSelection()
            ...
            If MsgBox("Dedese modificar el contrato N! " & txtIdContrato.Text & " ", vbYesNo) = vbYes Then
                ...
                If String.IsNullOrWhiteSpace(txtIdContrato.Text) = False Then
                        ...
                        If ValidarCamposLLenos() Then
                            If CalcularTotales() Then
                                ...
                                ClaseModeloContratos.RegistrarContrato(txtIdContrato.Text, txtNumeroContrato.Text, cboVigencia.Text, cboTipoDocuContrato.Text, txtNumeroDocumento.Text, txtSucursal.Text,
                                                                                   cboTipoContrato.SelectedValue, Format(DtFechaInicial.Value, "yyyy/MM/dd"), Format(DtFechaFinal.Value, "yyyy/MM/dd"), txt0bjetoContrato.Text, txtValorSinIva.Text, txtValorIva.Text, txtValorPazYSalvo.Text,
                                                                                   txtValorLegalizacion.Text, txtNumeroDePagos.Text, cboExigePoliza.SelectedIndex, txtNombreEntidadPoliza.Text, Format(DtFechaExpedicionPoliza.Value, "yyyy/MM/dd"), cboVigente.SelectedIndex, txtVisitasMes.Text, Format(DtFechaCotizacion.Value, "yyyy/MM/dd"),
                                                                                    cboTipoDocuRepresentante.Text, cboNombreRepresentanteLegal.SelectedValue, lblCodigoUsuario2.Text, Format(ftRegis.Value, "yyyy/MM/dd"), lblCodigoUsuario2.Text, Format(ftModi.Value, "yyyy/MM/dd"), DataGridDetalleCuotas, DataGridDetallePolizas, txtCodPoliza.Text, cboVezPoliza.SelectedIndex + 1)
                                CargarDataGridContratos()
                            End If
                        End If
                    ...
                    Catch ex As Exception
                        Titulo01 = "Control de errores de ejecución"
                        Informa = "Lo siento pero se ha presentado un error" & Chr(13) & Chr(10)
                        Informa += "Error al actualizar el contrato" & Chr(13) & Chr(10)
                        Informa += "Mensaje del error: " & ex.Message
                        MessageBox.Show(Informa, Titulo01, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End Try
                End If
    ...
    Private Sub txtValorTotal_KeyDown(sender As Object, e As KeyEventArgs) Handles txtValorTotal.KeyDown
        Try
            Select Case e.KeyData
                Case Keys.Enter
                    If String.IsNullOrWhiteSpace(txtValorSinIva.Text) = True Or String.IsNullOrWhiteSpace(txtPorceIVA.Text) = True Then
                        MsgBox("El campo Valor sin IVA o %IVA esta vacio")
                        txtValorTotal.Clear()
                    Else
                        Dim ValorSinIva As Double = Convert.ToDouble(txtValorSinIva.Text)
                        Dim IvaCantidad As Double
                        Dim ValorTotal As Double
                        IvaCantidad = Math.Round(((ValorSinIva * Convert.ToInt16(txtPorceIVA.Text)) / 100), 0)
                        ValorTotal = ValorSinIva + IvaCantidad
                        ValorTotal = Math.Round((ValorTotal), 0)
        ...
        Try
            ...
            Select Case e.KeyData
                Case Keys.Enter
                    ...
                    Else
                        If String.IsNullOrWhiteSpace(txtIdContrato.Text) = True Then
                            CargarDataGridDetalleCoutas()
                        ElseIf String.IsNullOrWhiteSpace(txtIdContrato.Text) = False Then
    ...
    Private Sub txtValorSinIva_KeyDown(sender As Object, e As KeyEventArgs) Handles txtValorSinIva.KeyDown
        Try
            Select Case e.KeyData
                Case Keys.Enter
                    If String.IsNullOrWhiteSpace(txtValorSinIva.Text) = True Or String.IsNullOrWhiteSpace(txtPorceIVA.Text) = True Then
...
#Region "Combobox"
    ...
    Public Sub Cargarcombobox()
        ...
        Try
            ...
            Dim NombreContratante As DataSet = SQLDataSET("SELECT IdenProve, IdenProve + ' ' + RazonSol as Display FROM [GEOGRAXPSQL].[dbo].[Datos proveedores] WHERE HabilPro = 1")
            cboNombreContratante.DataSource = NombreContratante.Tables(0)
            cboNombreContratante.DisplayMember = "Display"
    ...
    Public Sub CargaDatosRepresentanteLegal(Identificacion As String)
        Try

            If String.IsNullOrWhiteSpace(Identificacion) = Nothing Then

                Dim DatosRepreLegal As SqlDataReader = SQLReader("SELECT TipDocRe, NumDocRe FROM [Datos del representante legal] WHERE NumDocRe = '" & Identificacion & "'")

                If DatosRepreLegal.HasRows = False Then
                    cn.Close()
                    Exit Sub
                Else
                    DatosRepreLegal.Read()

                    cboTipoDocuRepresentante.Text = DatosRepreLegal("TipDocRe")
                    txtDocumentoRepresentante.Text = DatosRepreLegal("NumDocRe")

                    DatosRepreLegal.Close()
                    cn.Close()
                End If
            End If
        ...
        Catch ex As Exception
            Titulo01 = "Control de errores de ejecución"
            Informa = "Lo siento pero se ha presentado un error" & Chr(13) & Chr(10)
            Informa += "En la funcion datos del representante legal" & Chr(13) & Chr(10)
            Informa += "Mensaje del error: " & ex.Message
            MessageBox.Show(Informa, Titulo01, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Public Sub CargaDatosCombobox()
        ...
        Try
            If bandera = 1 Then
                Dim DatosProveedor As SqlDataReader = SQLReader("SELECT IdenProve, TipoDocu, SucurProv, DigVeri  FROM [GEOGRAXPSQL].[dbo].[Datos proveedores] WHERE IdenProve = '" & cboNombreContratante.SelectedValue & "'")

                If DatosProveedor.HasRows = False Then
                    cn.Close()
                    Exit Sub
                Else
                    DatosProveedor.Read()

                    cboTipoDocuContrato.Text = DatosProveedor("TipoDocu")
                    txtNumeroDocumento.Text = DatosProveedor("IdenProve")
                    txtSucursal.Text = DatosProveedor("SucurProv")
                    txtDigiVeriContra.Text = If(IsDBNull(DatosProveedor("DigVeri")) = True, 0, DatosProveedor("DigVeri"))
    ...
    Private Sub cboNombreRepresentanteLegal_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboNombreRepresentanteLegal.SelectedIndexChanged
        If BanderaComboBoxPrincipales = 1 And bandera = 1 Then
            CargaDatosRepresentanteLegal(cboNombreRepresentanteLegal.SelectedValue)
        End If
    End Sub 'Con este evento llama a una funcion la cual carga el value en otros combobox
...
#Region "Funciones"
    ...
    Private Function GenerarConcecutivo() As Boolean
        Try
            ...
            If String.IsNullOrWhiteSpace(txtIdContrato.Text) = True Or txtIdContrato.Text = "0" Or txtIdContrato.Text = "-2" Or txtIdContrato.Text = "-3" Then
                Return False
            Else
                Return True
            End If
    ...
    End Function  'Funcion que genera el concecutivo
    ...
    Private Function Buscar(id As String) As DataRow
        ...
        Try
            ...
            cmd = New SqlCommand("SELECT * FROM [BDADYSNET].[dbo].[Datos registros de contratos] WHERE ID_Contratos = @id", cn)
    ...
    Public Sub CargarDataGridContratos()
        ...
        Try
            ...
            Else
                cn.Close()
                MsgBox("No hay contratos registrados")
            End If
    ...
    Public Sub BuscarDataGridContratos()
        Try
            ...
            Dim Contratos As SqlDataReader
            Contratos = SQLReader("Select dgc.ID_Contratos, dgc.TipDocContra, dgc.NumDocContra,  dp.RazonSol, dtc.NomTipCon FROM [Datos registros de contratos] as dgc, [GEOGRAXPSQL].[dbo].[Datos proveedores] as dp,  [Datos tipos de contratos] as dtc WHERE dgc.NumDocContra = dp.IdenProve and dgc.TipoContra = dtc.CodTipCon and (dgc.ID_Contratos like '%' + '" & Convert.ToInt64(txtBuscarContratos.Text) & "' + '%'  OR dgc.NumDocContra like '" & Convert.ToInt64(txtBuscarContratos.Text) & "' + '%') ")
            bandera = 0
    ...
    Private Sub MostrarTexbox(identificador)
        ...
        Try
            ...
            cboNombreContratante.SelectedValue = Convert.ToString(row("NumDocContra"))  'Esto dispara el evento que me carga los demas datos del proveedor
            CargaDatosCombobox()
            ...
            lblCodiModiPor.Text = Convert.ToString(row("CodModi"))

            lblCodiRegisPor.Text = Convert.ToString(row("CodRegis"))

            NombreUsuarioRegistro(lblCodiRegisPor.Text)
    ...
    Private Sub NombreUsuarioRegistro(codigo)
        Try
            ...
            If UserLogeado.HasRows = False Then
                MsgBox("No se encontro el nombre del usuario que registro este contrato en la base de datos")
                cn.Close()
            Else
                UserLogeado.Read()
    ...
    Private Sub NombreUsuarioModifico(codigo)
        Try
            Dim UserLogeado As SqlDataReader = SQLReader("select NombreUsa + Apellido1Usa as usuario from [DATUSIIGXPSQL].[dbo].[Datos usuarios de los aplicativos] where CodigoUsa = '" & codigo.ToString & "' ")
            If UserLogeado.HasRows = False Then
                MsgBox("No se encontro el nombre de el usuario que modifico este contrato en la base de datos")
                cn.Close()
            Else
                UserLogeado.Read()
    ...
    Public Function CalcularTotales() As Boolean
        ...
        sumaPorcentaje = Math.Round((sumaPorcentaje), 0)
        sumaValorTotal = Math.Round((sumaValorTotal), 0)
        ...
        If sumaValorTotal <> txtValorTotal.Text Then
            MsgBox("La suma de las cuotas no da el total: " & txtValorTotal.Text & " la suma esta dando " & sumaValorTotal, MsgBoxStyle.Information, "Control de detalles de cuotas")
            estado = False
            Return estado
        Else
            estado = True
...
#Region "DataGrid"
        ...
        Try
            ...
            If txtNumeroDePagos.Text > Resultado Then
                ...
                If reader.HasRows Then
                    ...
                    Dim ValorCuota As Double = Math.Round((TtlCuotasPagadas / AddFilas), 2)
                    Dim PorcentageDeCouta As Double = Math.Round(((ValorCuota * TtlPorcentajeCuotas) / TtlCuotasPagadas), 2)
                    Dim IndiceDeCuota As Int16 = Resultado
                    ValorCuota = Math.Round((ValorCuota), 2)
                    PorcentageDeCouta = Math.Round((PorcentageDeCouta), 2)
    ...
    Private Sub CargarDataGridDetalleCoutas()
        ...
        If ValidarCamposDataGrid() Then
            ...
            Dim ValorTotal As Double
            ValorTotal = ValorSinIva + IvaCantidad
            Dim ValorCuota As Double = Math.Round((ValorTotal / txtNumeroDePagos.Text), 2)
            Dim PorcentageDeCouta = Math.Round(((ValorCuota * 100) / ValorTotal), 2)
            ValorCuota = Math.Round((ValorCuota), 2)
            PorcentageDeCouta = Math.Round((PorcentageDeCouta), 2)
            For indice As Integer = 1 To txtNumeroDePagos.Text

                DataGridDetalleCuotas.Rows.Add(New String() {indice, Fecha.Date, PorcentageDeCouta, ValorCuota, False})
