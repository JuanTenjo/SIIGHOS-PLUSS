...
Public Class FrmInfNotaDebito
    Private Sub FrmInfNotaDebito_Load(sender As Object, e As EventArgs) Handles MyBase.Load


        ModuloVariablesAplicacion.Data = ""
        ModuloVariablesAplicacion.Data = "SELECT df.CodEstaDian, df.CodCuDe FROM [BDADYSNET].[dbo].[Datos notas debitos facturas] as df 
                                              WHERE df.PrefiNotDeb = '" & ModuloVariablesAplicacion.PrefiNotDeb & "' AND df.NumNoDe = '" & ModuloVariablesAplicacion.NumNoDeb & "' AND df.DocuDebito = '" & ModuloVariablesAplicacion.NumFact & "'"
        ...
        Dim VistaCuerpoFactura As DataSet = SQLDataSET("SELECT * FROM [BDADYSNET].[dbo].[Vista_Nota_Debito] WHERE (NumNoDe = '" + ModuloVariablesAplicacion.NumNoDeb + "') And (PrefiNotDeb = '" + ModuloVariablesAplicacion.PrefiNotDeb + "') AND (NumFact = '" + ModuloVariablesAplicacion.NumFact + "') AND (PrefiFact = '" + ModuloVariablesAplicacion.PrefiFact + "')")
        ...
        ModuloVariablesAplicacion.Data = "SELECT SUM([TotalDeta] + [ValIvaFac]) TolSuma FROM [BDADYSNET].[dbo].[Vista_Nota_Debito]  WHERE (NumNoDe = '" + ModuloVariablesAplicacion.NumNoDeb + "')  AND (PrefiNotDeb = '" + ModuloVariablesAplicacion.PrefiNotDeb + "')  AND (NumFact = '" + ModuloVariablesAplicacion.NumFact + "')   AND (PrefiFact = '" + ModuloVariablesAplicacion.PrefiFact + "')"
