...
Public Class FrmInfFacturaE
    Private Sub FrmInfFacturaE_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        ModuloVariablesAplicacion.Data = ""
        ModuloVariablesAplicacion.Data = "SELECT df.CodEstaDian, df.CodCufe FROM [Datos facturas realizadas] as df 
                                              WHERE DF.PrefiFact = '" & ModuloVariablesAplicacion.Prefi & "' AND DF.NumFact = '" & ModuloVariablesAplicacion.NumFac & "' AND DF.NumRemi = '" & ModuloVariablesAplicacion.Remi & "'"

        Dim reader As SqlDataReader = SQLReader(ModuloVariablesAplicacion.Data)

        If reader.HasRows = False Then
            MsgBox("No se encontro esta factura")
            cn.Close()
            Exit Sub
        Else
            ...
            If reader("CodEstaDian") = "02" Or reader("CodEstaDian") = "03" Or reader("CodEstaDian") = "05" Then
                ModuloVariablesAplicacion.QR = ("https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey=" + reader("CodCufe"))
            Else
        ...
        Dim VistaCuerpoFactura As DataSet = SQLDataSET("SELECT * FROM [BDADYSNET].[dbo].[Vista_Factura_Elec] WHERE NumFac = '" & Prefi + NumFac & "' and NumRemi = '" & Remi & "' ")
        ...
        ModuloVariablesAplicacion.Data = "SELECT (SUM([CantDeta] * [ValUniDeta]) + SUM([ValIVADeta])) AS TolSuma  FROM [BDADYSNET].[dbo].[Vista_Factura_Elec] 
                                            WHERE NumRemi = '" & ModuloVariablesAplicacion.Remi & "' and NumFac = '" & ModuloVariablesAplicacion.Prefi + ModuloVariablesAplicacion.NumFac & "' "
